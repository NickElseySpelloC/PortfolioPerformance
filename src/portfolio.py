...
class PortfolioManager:
    ...
    def __init__(self, config, logger, price_data):
        ...
        self.reporting_period = {}  # To be initialised by set_reporting_period method based on config
    ...
    def set_reporting_period(self, units: str, quantity: int) -> dict:  # noqa: PLR0914, PLR0915
        """Sets the reporting period for the portfolio valuation.

        Args:
            units (str): The units of time for the reporting period: "days", "weeks", "months", "quarters", "years".
            quantity (int): The quantity of the time units for the reporting period, e.g. 7 for 7 days.

        Returns:
            reporting_period (dict): A dictionary containing the start and end dates of the reporting period, the units and quantity, and the number of days in the interval.
        """
        today = DateHelper.today()
        units_short = "d"

        if units == "days":
            end_date = DateHelper.today_add_days(-1)    # yesterday
            start_date = DateHelper.add_date(end_date, days=-quantity)
            units_short = "d"
        elif units == "weeks":
            # End date is the most recent Sunday (end of prior week)
            days_since_sunday = (today.weekday() + 1) % 7  # Monday=0, Sunday=6
            if days_since_sunday == 0:
                days_since_sunday = 7  # If today is Sunday, go back to prior Sunday
            end_date = DateHelper.add_date(today, days=-days_since_sunday)
            start_date = DateHelper.add_date(end_date, weeks=-quantity)
            units_short = "w"
        elif units == "months":
            # End date is the last day of the prior complete month
            first_of_this_month = today.replace(day=1)
            end_date = DateHelper.add_date(first_of_this_month, days=-1)
            # Calculate start_date: start of the month (quantity-1) months before end_date
            start_year = end_date.year
            start_month = end_date.month - (quantity - 1)
            while start_month <= 0:
                start_month += 12
                start_year -= 1
            ...
            # End date is the last day of the prior complete quarter
            ...
            if prior_quarter == 0:
                ...
                end_year = today.year
                end_month = prior_quarter * 3
            # Last day of the quarter's final month
            if end_month == 12:
                end_date = dt.date(end_year, 12, 31)
            else:
                next_month_first = dt.date(end_year, end_month + 1, 1)
                end_date = DateHelper.add_date(next_month_first, days=-1)

            # Calculate start_date: start of quarter, then go back (quantity-1) quarters
            quarter_of_end = (end_date.month - 1) // 3  # 0-indexed (0=Q1, 1=Q2, 2=Q3, 3=Q4)
            start_month_of_end_quarter = quarter_of_end * 3 + 1
            start_year = end_date.year
            start_month = start_month_of_end_quarter - (quantity - 1) * 3
            while start_month <= 0:
                start_month += 12
                start_year -= 1
            start_date = dt.date(start_year, start_month, 1)
            ...
            # End date is Dec 31 of the prior complete year
            end_date = dt.date(today.year - 1, 12, 31)
            # Start date is Jan 1 of (quantity) years before end_date's year
            start_date = dt.date(end_date.year - quantity + 1, 1, 1)
            units_short = "y"
        else:
            self.logger.log_fatal_error(f"Invalid reporting period units '{units}' specified in configuration.")
            return {}

        interval_days = DateHelper.days_between(start_date, end_date)
        self.reporting_period = {
            "StartDate": start_date,
            "StartDateStr": start_date.strftime("%d %B %Y"),
            "EndDate": end_date,
            "EndDateStr": end_date.strftime("%d %B %Y"),
            "IntervalUnits": units,
            "IntervalUnitsShort": units_short,
            "IntervalQuantity": quantity,
            "DaysDifference": interval_days,  # Number of days between StartDate and EndDate
            "ShortDescription": f"{quantity}{units_short}",
            "LongDescription": f"{quantity} {units[:-1]}",
        }
        self.report_name = self.config.get("Portfolio", "ReportName", default="Portfolio Performance Report")
        self.report_name += f" - {quantity} {units[:-1] if quantity == 1 else units}"
    ...
    def value_portfolio(self, mode: str) -> bool:
        ...
        self.price_misses = 0
        effective_date = self._get_effective_date(mode)
        ...
        for entry in self.holdings:
            ...
            if currency != self.reporting_currency:
                ...
                fx_rate, _ = self.price_data.get_price_on_date(yahoo_symbol, effective_date)
                if fx_rate is None:
                    self.logger.log_fatal_error(f"Failed to get {yahoo_symbol} FX rate on {effective_date}.")
            ...
            price, price_currency = self.price_data.get_price_on_date(symbol, effective_date)
            if price is None:
                self.price_misses += 1
                symbol_value = 0
            elif price_currency is not None and price_currency != currency:
                self.logger.log_message(f"Price currency mismatch for {symbol} on {effective_date}: expected {currency}, got {price_currency}. Using price as is.", "warning")
                self.price_misses += 1
                symbol_value = 0
            else:
        ...
        self.logger.log_message(f"Valuing portfolio as at {effective_date} at {self.reporting_currency_symbol}{self.value[mode]:,.0f}", "detailed")
    ...
    def send_text_report(self):
        ...
        assert isinstance(report_path, Path), "Report path must be a Path object"

        units_short = self.reporting_period.get("IntervalUnitsShort", "d")
        quantity = self.reporting_period.get("IntervalQuantity", 0)
        summary_message = f"Portfolio {quantity}{units_short} move: {self.display_cash(self.value['ValueChange'], "delta")} ({self.display_percentage(self.value['PcntChange'], "delta")}). "
    ...
    # ========== PRIVATE FUNCTIONS ================================================================================
    # TO DO: Change all the below method names to _method_name to indicate that they are private methods

    def _get_effective_date(self, mode: str) -> dt.date:
        """
        Gets the effective date for the specified valuation mode.

        Args:
            mode (str): The valuation mode, either "Current" or "Prior".

        Returns:
            effective_date (dt.date): The effective date for the specified valuation mode.
        """
        today = DateHelper.today()
        if mode not in {"Current", "Prior"}:
            self.logger.log_fatal_error(f"Invalid mode '{mode}' specified for getting effective date.")
            return today   # Just to keep linter quiet

        if not self.reporting_period:
            self.logger.log_fatal_error("Reporting period is not set. Cannot value portfolio.")
            return today   # Just to keep linter quiet

        if mode == "Current":
            return self.reporting_period.get("EndDate", today)
        return self.reporting_period.get("StartDate", today)
    ...
    def save_portfolio_valuation(self, mode: str) -> bool:
        ...
        if self.reporting_period is None or self.value[mode] is None:
        ...
        if self.portfolio_valuation_file.exists():
            with self.portfolio_valuation_file.open(newline="", encoding="utf-8") as csvfile:
                ...
                for row in reader:
                    if row:
                        row_date = DateHelper.extract_date(row[0])
                        if row_date is None:
                            # If the date is invalid, skip this row
                            continue

                        # Convert as_at_date to date if it's a datetime
                        if row_date != self._get_effective_date(mode):
                            existing_rows.append(row)

        # Add record for the specified valuation
        new_record = [
            DateHelper.format(self._get_effective_date(mode), "%Y-%m-%d"),
            f"{round(self.value[mode], 0)}",
        ]
        existing_rows.append(new_record)

        # Remove rows with invalid dates before sorting
        existing_rows = [row for row in existing_rows if DateHelper.extract_date(row[0]) is not None]
        # Sort the rows by date in ascending order, using only valid dates
        existing_rows.sort(key=lambda x: DateHelper.extract_date(x[0]) or dt.datetime.min)  # noqa: DTZ901
